...
function Folder(folderDescription, pageId,linkk) 
{   
  this.desc = folderDescription; 
  this.link = null==linkk?pageId+'.htm':linkk; 
  this.xID=pageId; 
  this.id = -1;
  this.navObj = 0;
  this.iconImg = 0; 
  this.nodeImg = 0;
  this.iconSrc = ICONPATH + "tvfolderopen.gif";
  this.iconSrcClosed = ICONPATH + "tvfolderclosed.gif";
...
{ 
  ...
  if (this.level>0) 
    if (this.isLastNode) 
      ...
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "tvvertline.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>";
...
function nodeImageSrc() {
  ...
  { 
    if (this.nChildren == 0)
      srcStr = ICONPATH + "tvlastnode.gif";
    else
      if (this.isOpen)
        srcStr = ICONPATH + "tvmlastnode.gif" ; 
      else
        srcStr = ICONPATH + "tvplastnode.gif"  ;
  } 
  else 
  { 
    if (this.nChildren == 0)
...
function finalizeCreationOfItem(itemArray)
{
  itemArray.navObj = 0 ;
  itemArray.iconImg = 0;
  itemArray.iconSrc = ICONPATH + "tvdoc.gif" ;
...
{ 
  ...
  var fullLink = "href='#' onClick=\"clickOnLink('"+this.getID()+"\', '"+this.link+"','"+this.target+"');return false;\"";
  this.isRendered = 1;

  if (this.level>0) 
    if (this.isLastNode) 
    { 
      leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "tvlastnode.gif' width=16 height=22></td>";
    } 
    else 
    { 
      leftSide = leftSide + "<td valign=top background=" + ICONPATH + "tvvertline.gif><img src='" + ICONPATH + "tvnode.gif' width=16 height=22></td>";
    } 
...
function leftSideHTML(leftSideCoded) {
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top background=" + ICONPATH + "tvvertline.gif><img src='" + ICONPATH + "tvvertline.gif' width=16 height=22></td>";
		}
...
function findObj(id,instr)
{
  ...
  if (typeof foldersTree.xID != "undefined") {
    ...
    if (null != instr && instr) 
    {
        for (i = 0; i < nEntries && indexOfEntries[i].xID .indexOf(id)==-1; i++)
            ;
    }
    else
    */
    {
        for(i=0;i<nEntries&&!indexOfEntries[i].xID.endWith(id);i++) 
          ;
    }
    id = i;
...
function highlightObjLink(nodeObj) {
 ...
 if(lastClicked != nodeObj)
  {
    lastClicked = nodeObj;     
    window.open(lastClicked.link+(lastClicked.link.indexOf("?")!=-1?"&":"?")+"pageid="+lastClicked.xID,lastClicked.target);    
  }
  if (PRESERVESTATE)
...
function gFld(description, pageId,link) 
{ 
  folder = new Folder(description, pageId,link);
  return folder;
} 
 
function gLnk(optionFlags, description,pageId, linkData) 
{ 
  if (optionFlags>=0) { 
  ...
  setItemLink(newItem, optionFlags, pageId,linkData);
  return newItem;
} 

function setItemLink(item, optionFlags,pageId, linkData) {
  var targetFlag = "";
  var target = "";
  var protocolFlag = "";
  var protocol = "";
  ...
  item.link = protocol+(null==linkData?pageId+".htm":linkData);      
  item.target = target;
  item.xID=pageId;
}
